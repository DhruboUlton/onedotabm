...
export async function getAnalyticsOverview(): Promise<AnalyticsOverview> {
  ...
  const sourcesRes = await dbQuery<{ source: string; count: string }>(`
  ...
  const totalProjectsNum = Number(kpi?.total_projects) || 0;
  const statusRes = await dbQuery<{ status: string; count: string }>(`
  ...
  const monthlyRes = await dbQuery<{ month: string; revenue: string; count: string }>(`
    SELECT 
  ...
  // 5. Top Clients
  const clientsRes = await dbQuery<{
    id: string;
    name: string;
    revenue: string;
  ...
  // 6. Recent Activities
  const activitiesRes = await dbQuery<ActivityRecord>(`
